...
export default function LenderPortal() {
  ...
  const [bidDone, setBidDone]       = useState<Set<number>>(new Set());
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (lenderAddr) {
        ...
        if (lr.ok) setLenderData(await lr.json());
      }
    } catch {
      setAuctions([]);
    } finally {
  ...
  async function handleDeposit() {
    ...
    if (!amount || amount < 5) { setDepositResult("error"); setDepositMsg("Minimum deposit is $5 USDC"); return; }
    setDepositLoading(true); setDepositResult(""); setDepositMsg("");
    try {
      await fetch(`${API}/api/lenders/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ address: lenderAddr }) });
      ...
      setDepositMsg(`$${n(amount)} USDC recorded in your Arc lending pool balance.`);
      setDepositAmount("");
      setTimeout(fetchData, 600);
    } catch (e: any) {
      setDepositResult("error"); setDepositMsg(e.message);
    } finally {
  ...
  async function handleCCTP() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "CCTP initiation failed");
      setCctpData(data);
      setCctpResult("info");
    } catch (e: any) {
      setCctpResult("error");
  ...
  async function placeBid(invoiceId: number) {
    ...
    setBidding(invoiceId); setBidErrors(p => ({ ...p, [invoiceId]: "" }));
    try {
      await fetch(`${API}/api/lenders/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ address: lenderAddr }) });
      const res = await fetch(`${API}/api/lenders/bid`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lenderAddress: lenderAddr, invoiceId, amount, discountBps: Math.round(discountPct * 100) }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Bid failed");
      setBidDone(p => new Set([...p, invoiceId]));
      setTimeout(fetchData, 600);
    } catch (e: any) {
      ...
      <div style={{ padding: "clamp(20px,4vw,36px) clamp(16px,4vw,40px)", maxWidth: 560, margin: "0 auto", paddingTop: "10vh" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{
            ...
            margin: "0 auto 20px", boxShadow: "0 0 24px rgba(0,102,255,0.4)",
          }}>
          ...
          <p style={{ fontSize: 14, color: "var(--text-2)", marginBottom: 28, lineHeight: 1.6 }}>
            Deposit USDC, bid on live invoice auctions, and earn yield when invoices settle. Connect your Arc-compatible wallet to get started.
          </p>
          <div style={{ display: "flex", justifyContent: "center" }}>
            <ConnectButton />
          </div>
          <p style={{ fontSize: 12, color: "var(--text-3)", marginTop: 16 }}>
            Supports MetaMask, Coinbase Wallet, WalletConnect. Arc Testnet, chain 5042002.
          </p>
        </div>
      </div>
    );
    ...
    <div style={{ padding: "clamp(20px,4vw,36px) clamp(16px,4vw,40px)", maxWidth: 1100, margin: "0 auto" }}>
      ...
      {fxRates.length > 0 && (
        <div style={{ ...CARD, marginBottom: 20, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <div style={{ width: 20, height: 20, borderRadius: 5, background: "var(--arc)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <svg width="10" height="10" viewBox="0 0 200 200" fill="none"><path d="M100 10L180 160H20L100 10Z" stroke="white" strokeWidth="20" strokeLinejoin="round" fill="none"/><path d="M55 135H145" stroke="white" strokeWidth="18" strokeLinecap="round"/></svg>
            </div>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--arc-light)", letterSpacing: "0.05em" }}>CIRCLE STABLEFX RATES</span>
          </div>
          {fxRates.map(r => (
            <div key={`${r.fromCurrency}${r.toCurrency}`} style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <span style={{ fontSize: 10, color: "var(--text-3)", background: "var(--surface-2)", padding: "1px 6px", borderRadius: 4 }}>
                {r.source === "circle" ? "live" : "sandbox"}
              </span>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16, marginBottom: 24 }}>
        <div style={CARD}>
          ...
          <p style={{ fontSize: 12, color: "var(--text-2)", marginBottom: 16, lineHeight: 1.5 }}>Add funds to your Arc lending pool balance. Minimum $5 USDC for testing.</p>
          ...
          {depositResult === "success" ? (
            ...
            <>
              {depositResult === "error" && (
                <div style={{ background: "var(--danger-dim)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: 8, padding: "10px 12px", marginBottom: 12, fontSize: 12, color: "var(--danger)" }}>
                  {depositMsg || "Deposit failed. Try again."}
                </div>
              )}
              ...
              <div style={{ position: "relative", marginBottom: 12 }}>
                ...
                <input style={{ ...FIELD, paddingLeft: 28 }} onFocus={onFocus} onBlur={onBlur}
                  type="number" min="5" step="1" placeholder="100"
                  value={depositAmount} onChange={e => setDepositAmount(e.target.value)} />
              </div>
              <button className="btn-primary" style={{ width: "100%", justifyContent: "center" }}
                onClick={handleDeposit} disabled={depositLoading || !depositAmount || parseFloat(depositAmount) < 5}>
        ...
        <div style={CARD}>
          ...
          <p style={{ fontSize: 12, color: "var(--text-2)", marginBottom: 16, lineHeight: 1.5 }}>
            Bridge testnet USDC to Arc from another testnet using Circle CCTP. All chains below are testnets only.
          </p>
          ...
          {cctpResult === "info" && cctpData ? (
            <div className="info-box">
              ...
              <p style={{ marginBottom: 8 }}>Call <code style={{ background: "rgba(0,0,0,0.3)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>depositForBurn()</code> on {cctpData.tokenMessenger}</p>
            ...
            <div style={{ background: "var(--danger-dim)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: 8, padding: "10px 12px", marginBottom: 12, fontSize: 12, color: "var(--danger)" }}>
              CCTP initiation failed. Check your connection and try again.
              <button className="btn-ghost" style={{ display: "block", marginTop: 8, fontSize: 11 }} onClick={() => setCctpResult("")}>Retry</button>
            </div>
          ) : (
            <>
              ...
              <select style={{ ...FIELD, appearance: "none" as const, marginBottom: 10, backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%237d88a8' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E\")", backgroundRepeat: "no-repeat", backgroundPosition: "right 12px center", paddingRight: 32 }}
                ...
                value={cctpChain} onChange={e => setCctpChain(e.target.value)}>
              ...
              <div style={{ position: "relative", marginBottom: 12 }}>
                <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--text-3)", fontSize: 13, fontFamily: "var(--font-mono)", pointerEvents: "none" }}>$</span>
                <input style={{ ...FIELD, paddingLeft: 28 }} onFocus={onFocus} onBlur={onBlur}
                  type="number" min="5" step="1" placeholder="50"
                  value={cctpAmount} onChange={e => setCctpAmount(e.target.value)} />
              </div>
              <button className="btn-primary" style={{ width: "100%", justifyContent: "center" }}
                onClick={handleCCTP} disabled={cctpLoading || !cctpAmount || parseFloat(cctpAmount) < 5}>
      ...
      <div className="surface" style={{ overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", borderBottom: "1px solid rgba(0,102,255,0.12)", flexWrap: "wrap", gap: 8 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: "var(--text-2)", marginTop: 2 }}>Bid amounts use Arc opt-in privacy. Only you and the seller see your rate during the auction window.</p>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: "48px 24px", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 12, color: "var(--text-3)" }}>New auctions open when sellers submit invoices. Try submitting one from the seller portal to test.</p>
          </div>
        ) : (
          <div>
            {auctions.map(auction => {
              const dl = daysLeft(auction.due_date);
              const done = bidDone.has(auction.id);
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", flexWrap: "wrap", gap: 16, justifyContent: "space-between" }}>
                    <div style={{ flex: "1 1 200px" }}>
                      ...
                      {bidErrors[auction.id] && (
                        <p style={{ fontSize: 11, color: "var(--danger)", marginTop: 5 }}>{bidErrors[auction.id]}</p>
                      )}
                    </div>

                    {done ? (
                      <div className="badge badge-green" style={{ alignSelf: "center" }}>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
                        <div style={{ minWidth: 100 }}>
                          ...
                          <input style={FIELD} onFocus={onFocus} onBlur={onBlur}
                            type="number" min="5" step="1" placeholder="50"
                            value={bidAmounts[auction.id] || ""}
                            onChange={e => setBidAmounts(p => ({ ...p, [auction.id]: e.target.value }))} />
                        </div>
                        <div style={{ minWidth: 80 }}>
                          <label className="label">Discount (%)</label>
                          <input style={FIELD} onFocus={onFocus} onBlur={onBlur}
                            type="number" min="0.1" max="20" step="0.1" placeholder="2.4"
                            value={bidRates[auction.id] || ""}
                            onChange={e => setBidRates(p => ({ ...p, [auction.id]: e.target.value }))} />
                        </div>
                        <button className="btn-primary" onClick={() => placeBid(auction.id)}
                          disabled={isBidding || !bidAmounts[auction.id] || !bidRates[auction.id]}
                          style={{ whiteSpace: "nowrap" }}>
                          {isBidding ? <><span className="loading-spinner"/> Placing...</> : "Place bid"}
                        </button>
                      </div>
                    )}
                  </div>

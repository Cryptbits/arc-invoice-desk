...
export default function Sidebar() {
  ...
  const currentLabel = NAV.flatMap(g => g.items).find(i => i.href === path)?.label || "Menu";

  return (
    <aside>
      {/* Desktop sidebar */}
      <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "18px 12px 20px" }} className="desktop-sidebar">
      ...
      {/* Mobile top bar */}
      <div className="mobile-nav-header" style={{ display: "none" }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          <AppLogo size={28} />
          <div>
            <p style={{ fontWeight: 800, fontSize: 13, color: "#dde4ff", lineHeight: 1 }}>Arc Invoice Desk</p>
            <p style={{ fontSize: 9, color: "var(--arc-light)", fontWeight: 600, letterSpacing: "0.05em" }}>BUILT ON ARC</p>
          </div>
        </Link>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: connected ? "rgba(0,214,143,0.1)" : "var(--arc-dim)",
                    border: `1px solid ${connected ? "rgba(0,214,143,0.2)" : "rgba(0,102,255,0.2)"}`,
                    color: connected ? "var(--success)" : "var(--arc-light)",
          ...
          <button
            ...
            style={{ padding: "7px 10px", borderRadius: 7, background: "var(--surface-2)", border: "1px solid var(--border)", cursor: "pointer", display: "flex", flexDirection: "column", gap: 4, alignItems: "center" }}
          >
            {menuOpen ? (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 3L13 13M13 3L3 13" stroke="var(--text-2)" strokeWidth="1.6" strokeLinecap="round"/></svg>
            ) : (
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 4H14M2 8H14M2 12H14" stroke="var(--text-2)" strokeWidth="1.6" strokeLinecap="round"/></svg>
            )}
          </button>
        </div>
      </div>
      ...
      {/* Mobile nav menu */}
      <div className={`mobile-nav-body${menuOpen ? " open" : ""}`}>
        {NAV.map(group => (
          <div key={group.group} style={{ marginBottom: 4 }}>
            <p style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" as const, color: "var(--text-3)", padding: "8px 12px 4px" }}>{group.group}</p>
            {group.items.map(item => {
              const active = path === item.href;
              return (
                <Link key={item.href} href={item.href} className={`nav-item${active ? " active" : ""}`}
